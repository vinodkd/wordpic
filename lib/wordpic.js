  ...
  return {
    ...
    normalize:function(objects){
      ...
      function calcBBox(objects){
		    var bbox = {rmin:0,cmin:0,rmax:0,cmax:0,h:0,w:0};
				for(var o in objects){
					var obj=objects[o];
					if(obj.r < bbox.rmin) bbox.rmin=obj.r;
					if(obj.c < bbox.cmin) bbox.cmin=obj.c;
					if(obj.r > bbox.rmax) bbox.rmax=obj.r;
					if(obj.c > bbox.cmax) bbox.cmax=obj.c;
				}
				bbox.h=bbox.cmax - bbox.cmin;
				bbox.w=bbox.rmax - bbox.rmin;
        ...
        return bbox;
			...
			function xlateToOrigin(objects,bbox){
				for(var o in objects){
					var obj=objects[o];
					obj.r = obj.r - bbox.rmin;
					obj.c = obj.c - bbox.cmin;
				}
				bbox.rmin=0; bbox.cmin=0;
				bbox.rmax=bbox.w; bbox.cmax=bbox.h;
    ...
    render:function(lspace){
      ...
      console.log("render");
      console.log("size of pic:" + lspace.bbox.rmax + "," + lspace.bbox.cmax);
      for(var objname in lspace.objects){
        var obj = lspace.objects[objname];
        console.log(objname + ':\t' + obj.r+","+obj.c +"\tdim:" + obj.h + "," + obj.w);
      }      
    }
  };
}());
